document.addEventListener("DOMContentLoaded", () => {
  ...
  const baseYAxis = [oilAxis, fluidAxis, fluidAxis];
  ...
  let chart = new ApexCharts(chartElement, {
    ...
    events: {
      selection: function (chartContext, { xaxis }) {
        ...
        const selectedData = chartContext.opts.series[0].data.filter(
          (point) => {
  ...
  function updateSelectedPredictData(data) {
    ...
    if (currentState !== "prediction") {
      const currentDataPointIndexes = selectedPredictData.map(
        (it) => it.dataPointIndex
      );
      if (currentDataPointIndexes.includes(data.dataPointIndex)) {
        // remove data
        selectedPredictData = selectedPredictData.filter(
          (it) => it.dataPointIndex !== data.dataPointIndex
        );
      } else {
        // add data
  ...
  function updateSelectedPredictView() {
    ...
    const filteredSeries = currentDataSeries.filter(
      (it) => it.name !== "Selected Automatic Data"
    );
    const newSeries = [
      ...filteredSeries,
      {
  ...
  function updateSelectedPredictObjectView() {
    ...
    const filteredSeries = currentDataSeries.filter(
      (it) => it.name !== "Selected Prediction Data"
    );
    const newSeries = [
      ...filteredSeries,
      {
  ...
  const fetchHistory = (well, startDate, endDate) => {
    ...
    fetch("https://api-dca.ugmtest.icu/get_history", {
      ...
      .then((data) => {
        ...
        currentDataSeries = [productionSeries, fluidSeries, jobCodeSeries];
  ...
  document.getElementById("automateDCA").addEventListener("click", function () {
    ...
    const selected_data =
      selectedPredictData?.map((it, index) => {
        return {
          Date: it.Date,
    ...
    fetch("https://api-dca.ugmtest.icu/automatic_dca", {
      ...
      .then((data) => {
        ...
        document.getElementById(
          "exp-decline"
        ).value = `${data.DeclineRate.Exponential}`;
        document.getElementById(
          "harm-decline"
        ).value = `${data.DeclineRate.Harmonic}`;
        document.getElementById(
          "hyper-decline"
        ).value = `${data.DeclineRate.Hyperbolic}`;
        ...
        const hyperbolicModel = (t, qi, b, n) =>
          qi * Math.pow(1 + b * t, -1 / n);
        ...
        const exponentialData = generatePrediction(
          exponentialModel,
          data.Exponential,
          startDate,
          endDate
        );
        const harmonicData = generatePrediction(
          harmonicModel,
          data.Harmonic,
          startDate,
          endDate
        );
        const hyperbolicData = generatePrediction(
          hyperbolicModel,
          data.Hyperbolic,
          startDate,
          endDate
  ...
  const fetchPredictionWithSelectedData = (selectedData, elr) => {
    ...
    fetch("https://api-dca.ugmtest.icu/predict_production", {
      ...
      .then((data) => {
        ...
        const newSeries = [
          {
            name: "Exponential Decline (Prediction)",
            type: "line",
            data: exponentialData,
            yAxisIndex: 0,
            hidden: false,
          ...
          {
            name: "Harmonic Decline (Prediction)",
            type: "line",
            data: harmonicData,
            yAxisIndex: 0,
            hidden: true,
          ...
          {
            name: "Hyperbolic Decline (Prediction)",
            type: "line",
            data: hyperbolicData,
            yAxisIndex: 0,
            hidden: true,
        ...
        const predictionSeries = [...currentDataSeries];
  ...
  document.getElementById("mlDCA").addEventListener("click", () => {
    ...
    fetch("https://api-dca.ugmtest.icu/predict_ml", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      ...
      .then((data) => {
        ...
        const filteredSeries = currentDataSeries.filter(
          (series) => !series.name.includes("ML Prediction")
        );
        const newSeries = [
          ...filteredSeries,
          actualSeries,
